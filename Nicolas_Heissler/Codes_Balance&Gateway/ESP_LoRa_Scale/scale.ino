...
/*
  ...
  do { // -999.9 < temperature factor < +999.9
    timerWrite(wdt_timer, 0); //reset timer (feed watchdog)
    if ( temp_factor > 1000 ) { // no value stored in eeprom -> init
      digit[0]=-13; digit[1]=-13; digit[2]=-13; digit[3]=-13; 
      digit[4] = '+';
    } else { // extract value stored i eeprom
      if ( temp_factor < 0.0 ) {
        digit[4] = '-';
        temp_factor *= -1.0;
      } else {
      ...
      digit[3] = (int8_t)( temp_factor / 100.0 ) ;
      digit[2] = (int8_t)( (temp_factor - (100.0 * digit[3]) ) / 10.0) ;
      digit[1] = (int8_t)( (temp_factor - (100.0 * digit[3]) - (10.0 * digit[2]) ) ) ;
      digit[0] = (int8_t)( round( (temp_factor - (int)(temp_factor) ) * 10.0 ) );
    }
    sprintf(buf,"%c%c%c%c.%c",digit[4],digit[3]+'0',digit[2]+'0',digit[1]+'0',digit[0]+'0');
    oled_print(OC16B2, TEMP_FACTOR1, buf + (String)" g/°", MASS3, MASS4);
    d = 4; 
    ...
    do {
        ...
        if ( next ) {
          if ( next_mem == 0 ) {
            next_mem = 1;
            if ( d == 4 ) { // signe
              digit[d] = (digit[d] == '+' ?  digit[d] = '-' : digit[d] = '+');
            } else { // digit
              digit[d]++;
              if ( digit[d] > 9 ) 
                digit[d] = 0; 
    ...
    oled_print(OC16B2, TEMP_FACTOR1, buf + (String)" g/°", "", "");
    temp_factor = (float)(digit[3]*100 + digit[2]*10 + digit[1] + digit[0]/10); // grams/°C
    if ( digit[4] == '-' )
      temp_factor *= -1;
    DEBUGPRINT3("temperature factor:"); DEBUGPRINT3(temp_factor); DEBUGPRINTLN3(" g/°C");
    delay(AFF_TIME);
  } while ( temp_factor > 1000 );
  */
  ...
  while ( !digitalRead(NEXT_BTN) ) {
    timerWrite(wdt_timer, 0); //reset timer (feed watchdog)
    oled_print(OB16, CAL1, CAL2 + (String)" A", String(scale0.getReading()), CAL4);
    delay(200);
  }
  while ( digitalRead(NEXT_BTN) );
  ...
  if ( sensor_nb > 1 ) {
    ...
    while ( !digitalRead(NEXT_BTN) ) {
      timerWrite(wdt_timer, 0); //reset timer (feed watchdog)
      oled_print(OB16, CAL1, CAL2 + (String)" B", String(scale0.getReading()), CAL4);
      delay(200);
    }
    while ( digitalRead(NEXT_BTN) );
  ...
  if ( sensor_nb == 4 ) {
    ...
    while ( !digitalRead(NEXT_BTN) ) {
      timerWrite(wdt_timer, 0); //reset timer (feed watchdog)
      oled_print(OB16, CAL1, CAL2 + (String)" C", String(scale1.getReading()), CAL4);
      delay(200);
    }
    while ( digitalRead(NEXT_BTN) );
    ...
    while ( !digitalRead(NEXT_BTN) ) {
      timerWrite(wdt_timer, 0); //reset timer (feed watchdog)
      oled_print(OB16, CAL1, CAL2 + (String)" D", String(scale1.getReading()), CAL4);
      delay(200);
    }
    while ( digitalRead(NEXT_BTN) );
...
uint8_t get_1sigma_Average (uint8_t sensor, int32_t *average_global, uint16_t samples, uint8_t readings) {
  ...
  for ( i = 0 ; i < readings ; i++ ) {
    ...
    //DEBUGPRINT3("average:"); DEBUGPRINTLN3(average[i]);
    sum = 0;
    ...
    average[i] = sum / k;
    sum_ave += average[i];
    //DEBUGPRINT3("average 1 s "); DEBUGPRINT3(i); DEBUGPRINT3(" :"); DEBUGPRINTLN3(average[i]);
  }
  ...
  *average_global = sum_ave / i;
  ...
  if ( calibrationFactor[sensor] ) { // measure mass
    ...
    //DEBUGPRINT3("sg:"); DEBUGPRINTLN3((int32_t)sqrt(sum / (readings-1)));
    if ( sqrt(sum / (readings-1)) < abs(*average_global/100) ) {
...
////*** **********************************************************
//uint8_t get_1sigma_Average (NAU7802 scale, uint8_t channel, int32_t *average_global, uint16_t samples, uint8_t readings) {
//  /* 
//   *  
//   */
//  uint8_t i = 0;
...
//  sum = 0;
//  for ( i = 0 ; i < readings ; i++ ) { // standard deviation of readings (ecart type)
//    delta = abs(average[i] - *average_global);
//    DEBUGPRINT3(" delta="); DEBUGPRINT3(delta);
//    //DEBUGPRINT3(" - average_global/10="); DEBUGPRINT3(*average_global/10);DEBUGPRINTLN3(" ## ");
//  if ( delta > DELTA_MASS_MAX ) { //abs(*average_global/10) ) {
//    Serial.println("ERROR");
//    for ( i = 0 ; i < readings ; i++ ) {DEBUGPRINT3(i);DEBUGPRINT3("="); DEBUGPRINT3(average[i]);}DEBUGPRINT3(" ");
//    return 1;
//  }
//  
//  }
