...
uint8_t  send_max_tries = 4;            // if the first data send has no acknoledge, number of resend tries, max SEND_MAX_TRIES_MAXI
// don't modify these values unless you understand all the code
#define  CAL_SAMPLES 255 // 16 bits
#define  CAL_READINGS 15 // 8  bits
#define  SAMPLES  50     // 16 bits
#define  READINGS 5      // 8  bits
#define  DELTA_MASS_MAX 500 // g
// ***********************************************************************
...
// datas
uint8_t actions_order[] = {0, 5, 7, 1, 2, 3, 4, 6, 8, 9, 10, 11};
...
void setup() {
  ...
  if ( esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_EXT0 ) { // wake up from VALID button
      ...
      if ( millis() > WAIT_TIME_FOR_CONFIG + previousTime ) {  // enter settings mode after WAIT_TIME_FOR_CONFIG
        ...
        do {
          if ( digitalRead(VALID_BTN) ) { // go to calibration
            ...
            oled_print(OB24, SETTING, "CALIBRATION", "", "");
            while ( digitalRead(VALID_BTN) ) { // wait VALID_BTN to be released
              delay(100);
            } 
    ...
    switch ( ds.action ) { //0:no_action,1:Sirup50/50,2:Sirup70/30,3:Candy,4:materiel,5:Visit,6:measure only,7:sleep,8:infos bat,9:version,10:lora,11:temp drift
      ...
      case 8 : // infos bat
        ...
        go_to_sleep (sleep_time, OLED_ON);
      case 9 : // version
        oled_print(OC24, String(INIT_MSG1) + " " + String(INIT_MSG2), "Ver. " + (String)version , SAMPLE_NB + (String)SAMPLES + "x" + (String)READINGS, "");
        delay(AFF_TIME);
        go_to_sleep (sleep_time, OLED_ON);
      case 10: // lora
        ...
        go_to_sleep (sleep_time, OLED_ON);
      case 11: // temp drift
        oled_print(OB16, (String)temp_factor + "g/°C" , TEMP_DRIFT1, TEMP_DRIFT2, TEMP_DRIFT3);
        delay(AFF_TIME);
        go_to_sleep (sleep_time, OLED_ON);      
  ...
  while ( get_1sigma_Average(SENSOR_0, &onScale, SAMPLES, READINGS) && i < get_ave_max_tries ) {  
    DEBUGPRINTLN3("error sensor A");
    error_flags |= 0b00010000;
    i++;
  }
  ds.mass = ((float)onScale-(float)zeroOffset[0])/calibrationFactor[0];
  DEBUGPRINT3(" mass 1: "); DEBUGPRINTLN3(((float)onScale-(float)zeroOffset[0])/calibrationFactor[0]);
  ...
  if ( sensor_nb > 1 ) {
    ...
    while ( get_1sigma_Average(SENSOR_1, &onScale, SAMPLES, READINGS) && i < get_ave_max_tries ) {  
      DEBUGPRINTLN3("error sensor B");
      error_flags |= 0b00010000;
      i++;
    }
    ds.mass += ((float)onScale-(float)zeroOffset[1])/calibrationFactor[1];
    DEBUGPRINT3(" mass 2: "); DEBUGPRINT3(((float)onScale-(float)zeroOffset[1])/calibrationFactor[1]);
    DEBUGPRINT3(" mass t: "); DEBUGPRINTLN3(ds.mass);
  }
  scale0.powerDown();
  ...
  if ( sensor_nb == 4 ) {
    ...
    while ( get_1sigma_Average(SENSOR_2, &onScale, SAMPLES, READINGS) && i < get_ave_max_tries ) {  
      DEBUGPRINTLN3("error sensor C");
      error_flags |= 0b00010000;
      i++;
    }
    ds.mass += ((float)onScale-(float)zeroOffset[2])/calibrationFactor[2];
    DEBUGPRINT3(" mass 3: "); DEBUGPRINT3(((float)onScale-(float)zeroOffset[2])/calibrationFactor[2]);
    DEBUGPRINT3(" mass t: "); DEBUGPRINTLN3(ds.mass);
    ...
    while ( get_1sigma_Average(SENSOR_3, &onScale, SAMPLES, READINGS) && i < get_ave_max_tries ) {  
      DEBUGPRINTLN3("error sensor D");
      error_flags |= 0b00010000;
      i++;
    }
    ds.mass += ((float)onScale-(float)zeroOffset[3])/calibrationFactor[3];
    DEBUGPRINT3(" mass 4: "); DEBUGPRINT3(((float)onScale-(float)zeroOffset[3])/calibrationFactor[3]);
    DEBUGPRINT3(" mass t: "); DEBUGPRINTLN3(ds.mass);
    scale1.powerDown();
  }
  ...
  ds.mass -= massOffset;
  DEBUGPRINT3("mass offset: "); DEBUGPRINTLN3(massOffset);
  ...
  if (ds.action)
    oled_print(OB24, String(buf_mass), "", "", "");

  // End measures
  DEBUGPRINT0("TIME End measures : "); DEBUGPRINT0(millis()); DEBUGPRINTLN0(" ms ***************");
...
void loop() {
  ...
  if ( millis() > previousTime + WAIT_TIME_FOR_ACKNOWLEDGE ) {
    LoRa_stop();
    DEBUGPRINT0("TIME LoRa stopped (timeout) : "); DEBUGPRINT0(millis()); DEBUGPRINTLN0(" ms ***************");
    ...
    if ( packetSize != ACKNOWLEDGE_LENGTH ) {  // ===== received a packet not acknowledge size 
      ...
      if ( myMACaddress ) { // ===== an acknowlege for me
        ...
        if ( ack_OK == 1 ) {  // ===== acknowledge OK => go to sleep
          LoRa_stop();
          DEBUGPRINT0("TIME LoRa stopped (ack OK): "); DEBUGPRINT0(millis()); DEBUGPRINTLN0(" ms ***************");
          // stop blinking led
          tickerSetLow.detach();
