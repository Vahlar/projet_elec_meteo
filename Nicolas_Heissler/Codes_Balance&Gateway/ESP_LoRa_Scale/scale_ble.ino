...
void create_ble_uart (uint8_t * macSTA) {
  ...
  while (!deviceConnected) {
      if ( millis() > (previousTimeSec*1000)+1000 ) {
        oled_print(OC16, BLE_CONNECT1, BLE_CONNECT2, BLE_CONNECT3, 
        (WAIT_TIME_FOR_BLE + previousTime - millis())/1000 + String("s")); 
...
int ble_select_time_param_id (uint16_t nb_id, char *sleep_time_type) {
  ...
  do {
    if ( sleep_time_values[nb_id] < 120 )
      ...
      sprintf(buf,"%s %s (%d mn)", SLEEP_TIME, sleep_time_type, sleep_time_values[nb_id]/60);
    write_to_ble_terminal (buf, LINE_FEED_ON);
...
//// ==============================
//void read_ble_terminal ( char *param, uint8_t len) {
//  
//  std::string rxValue;
//  uint8_t strTooLong = 0;
//  uint8_t i = 0;
//  
//  for (i = 0; i < len ; i++) // reset param
//      param[i] = '\0';
//  
//  do {
//    strTooLong = 0;
...
//    timerWrite(wdt_timer, 0); //reset timer (feed watchdog)  
//
//    DEBUGPRINT3("Received Value length: ");DEBUGPRINTLN3(rxValue.length());
//    if (rxValue.length() > PARAM_LENGTH-2) { // for '\0', CR, LF
//      strTooLong++;
//      write_to_ble_terminal(STRING_TO_LONG,LINE_FEED_ON); 
//      pRxCharacteristic->setValue(""); //  reset RxCharacteristic value
//    }
//  } while ( strTooLong );
// 
//  DEBUGPRINT3("Received Value: ");
//  for (i = 0; i < rxValue.length() ; i++) {
//    if ( !iscntrl(rxValue[i]) ) { // remove CR, LF, ....
//      param[i] = rxValue[i];
//      DEBUGPRINT3(rxValue[i]);
//    }
//  }
//  param[i] = '\0'; //'\0';
//  DEBUGPRINTLN3("\n");
...
//// ==============================
//void set_parameters() {
//  oled_print(OB16, CREATE_BLE1, CREATE_BLE2, CREATE_BLE3, CREATE_BLE4);
//  
//  DEBUGPRINTLN2("params not set ");
//  
//  create_ble_uart();
//
//  write_to_ble_terminal (SELECT_FREQ1, 1);
//  write_to_ble_terminal (SELECT_FREQ2, 1);
//  write_to_ble_terminal (SELECT_FREQ3, 1);
//  write_to_ble_terminal (SELECT_FREQ4, 1);
//  do {
//    read_ble_terminal ( ble_buf, sizeof(ble_buf),SELECT_FREQ5);
//    val = atoi(ble_buf);
//  } while ( val < CHANNEL_MIN || val > CHANNEL_MAX );
//  lr.freq = (long)(frequencies[val] * 1000000);
//  oled_print(OC24, "LoRa sf:", "f:" + String(lr.freq), "sbw:","");
//
//  write_to_ble_terminal (SELECT_SF1, 1);
//  do {
//    read_ble_terminal ( ble_buf, sizeof(ble_buf),SELECT_SF2);
//    val = atoi(ble_buf);
//  } while ( val < 7 || val > 12 );
//  lr.sf = (uint8_t)atoi(ble_buf);
//  oled_print(OC24, "LoRa sf:" + String(lr.sf), "f:" + String(lr.freq), "sbw:","");
//
//  write_to_ble_terminal (SELECT_SBW1, 1);
//  do {
//    read_ble_terminal ( ble_buf, sizeof(ble_buf),SELECT_SBW2);
//    val = atoi(ble_buf);
//  } while ( val != 125 && val != 250 && val != 500 );
//  lr.sbw = (long)(atoi(ble_buf) * 1000);
//  oled_print(OC24, "LoRa sf:" + String(lr.sf), "f:" + String(lr.freq), "sbw:" + String(lr.sbw),"");
//
//  read_ble_terminal ( ble_buf, sizeof(ble_buf), VALID_DATA);
//  if ( ble_buf[0] == 'o' || ble_buf[0] == 'O' || ble_buf[0] == 'y' || ble_buf[0] == 'Y' ) {
//    DEBUGPRINT2("valid:");DEBUGPRINTLN2(ble_buf);
//    param_ok = true;
//    eeprom_address = 0;
//
//    EEPROM.writeLong(eeprom_address, lr.freq);
//    eeprom_address += sizeof(long);
//    DEBUGPRINT3("lr.freq = ");DEBUGPRINTLN3(lr.freq);
//    EEPROM.writeLong(eeprom_address, lr.sbw);
//    eeprom_address += sizeof(long);
//    DEBUGPRINT3("lr.sbw = ");DEBUGPRINTLN3(lr.sbw);
//    EEPROM.writeUChar(eeprom_address, lr.sf);
//    eeprom_address += sizeof(unsigned char);
//    DEBUGPRINT3("lr.sf = ");DEBUGPRINTLN3(lr.sf);
//    EEPROM.commit();    
//  } else {
//    DEBUGPRINT2("valid:");DEBUGPRINTLN2(ble_buf);
//    DEBUGPRINTLN2(REBOOT);
//    delay(1000);
//    ESP.restart();
//  }
//}
